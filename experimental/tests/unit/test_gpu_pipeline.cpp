...
TEST(GpuPipelineTest, FillRectCommand) {
    ...
    // Outside the rect (default garbage or 0, depending on allocator, but since we didn't clear, we just check relative
    // difference or assume we can't test outside strictly if not 0 initialized. Wait, new[] doesn't zero-initialize,
    // let's zero it first).
