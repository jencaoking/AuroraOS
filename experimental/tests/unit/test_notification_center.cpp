...
// 测试辅助：构建标准 TLV 报文
// =============================================================
static void append_tlv(uint8_t* buf, uint8_t& len, uint8_t tag, const uint8_t* val, uint8_t val_len) {
    buf[len++] = tag;
    buf[len++] = val_len;
...
static Notification build_notification(uint32_t id, NotificationPriority p, NotificationCategory cat, const char* title,
                                       const char* body, uint32_t tick = 1000) {
    uint8_t buf[128]{};
    uint8_t len = 0;

    const uint8_t id_bytes[4] = {static_cast<uint8_t>(id & 0xFF), static_cast<uint8_t>((id >> 8) & 0xFF),
                                 static_cast<uint8_t>((id >> 16) & 0xFF), static_cast<uint8_t>((id >> 24) & 0xFF)};
    ...
    while (title[tlen] && tlen < Notification::kTitleMaxLen - 1u) {
        title_bytes[tlen] = static_cast<uint8_t>(title[tlen]);
        ++tlen;
    }
    ...
    while (body[blen] && blen < Notification::kBodyMaxLen - 1u) {
        body_bytes[blen] = static_cast<uint8_t>(body[blen]);
        ++blen;
    }
...
TEST_F(BleParserTest, ParsesAllTlvFieldsCorrectly) {
    const Notification n = build_notification(0xDEADBEEF, NotificationPriority::high, NotificationCategory::message,
                                              "Alice", "Hey, are you coming?", 5000);
...
TEST_F(QueueTest, PopAlwaysReturnsHighestPriority) {
    ...
    Notification out;
    ASSERT_TRUE(q.pop(out));
    EXPECT_EQ(out.priority, NotificationPriority::critical);
    ASSERT_TRUE(q.pop(out));
    EXPECT_EQ(out.priority, NotificationPriority::high);
    ASSERT_TRUE(q.pop(out));
    EXPECT_EQ(out.priority, NotificationPriority::normal);
    ASSERT_TRUE(q.pop(out));
    EXPECT_EQ(out.priority, NotificationPriority::low);
}
...
protected:
    void SetUp() override {
        ...
        // 初始状态：overlay 不在展示
        ON_CALL(*mock_overlay_, is_visible()).WillByDefault(::testing::Return(false));
    ...
    Notification make_notif(NotificationPriority p, uint32_t ts = 1000) {
        return build_notification(ts, p, NotificationCategory::message, "Test", "Body", ts);
    }
...
TEST_F(NotificationCenterTest, PostWhenVisibleQueuesButDoesNotShow) {
    // overlay 已在展示
    ON_CALL(*mock_overlay_, is_visible()).WillByDefault(::testing::Return(true));
...
TEST_F(NotificationCenterTest, OnTickDrivesBannerAutoDismissAndDispatch) {
    ...
    bool visible = true;
    ON_CALL(*mock_overlay_, is_visible()).WillByDefault([&visible]() {
        return visible;
    });
    ON_CALL(*mock_overlay_, tick(3001u)).WillByDefault([&visible](uint32_t) {
        visible = false;
    });
...
TEST_F(NotificationCenterTest, FullQueueReturnsFalseOnExcess) {
    ...
    // 填满队列
    for (int i = 0; i < PriorityNotificationQueue::kCapacity; ++i) {
        EXPECT_TRUE(NotificationCenter::instance().post(make_notif(NotificationPriority::normal)));
    }
    // 第 9 条应被拒绝
    EXPECT_FALSE(NotificationCenter::instance().post(make_notif(NotificationPriority::normal)));
}
