...
protected:
    ...
    const uint8_t select_transit[12] = {0x00, 0xA4, 0x04, 0x00, 0x07, 0xA0, 0x00, 0x00, 0x00, 0x03, 0x10, 0x10};

    const uint8_t select_door[10] = {0x00, 0xA4, 0x04, 0x00, 0x05, 0xF0, 0x00, 0x00, 0x00, 0x01};

    // CLA=0x80 INS=0x50 P1=0x00 P2=0x00 Lc=0x04 Data=TxID(0x00,0x00,0x00,0x01)
    const uint8_t debit_tx_1[9] = {0x80, 0x50, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x01};

    const uint8_t debit_tx_2[9] = {0x80, 0x50, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x02};

    const uint8_t debit_tx_0[9] = {0x80, 0x50, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00};
};
...
TEST_F(NfcServiceTest, UnknownCardReturnsFileNotFound) {
    const uint8_t select_unknown[] = {0x00, 0xA4, 0x04, 0x00, 0x05, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE};
...
TEST_F(NfcServiceTest, SelectApduLcByteMismatch) {
    ...
    const uint8_t select_transit_bad_lc[] = {0x00, 0xA4, 0x04, 0x00, 0x08, 0xA0, 0x00,
                                             0x00, 0x00, 0x03, 0x10, 0x10, 0x00};

    ApduResponse response;
    NfcController::instance().simulate_field_on(NfcTagType::TYPE_A);

    bool handled = NfcController::instance().simulate_incoming_apdu(select_transit_bad_lc,
                                                                    sizeof(select_transit_bad_lc), response);
    EXPECT_TRUE(handled);
    EXPECT_EQ(response.length, 2);
    EXPECT_EQ(response.data[0], 0x6A);
