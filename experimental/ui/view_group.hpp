...
public:
    ...
    virtual void fill_round_rect(int32_t x, int32_t y, uint32_t w, uint32_t h, uint32_t radius, uint16_t color) {}

    virtual void fill_rect(int32_t x, int32_t y, uint32_t w, uint32_t h, uint16_t color) {}

    virtual void draw_string(int16_t x, int16_t y, const char* text, uint8_t scale, uint16_t fg, uint16_t bg,
                             const uint8_t* font_data, uint8_t font_w, uint8_t font_h) {}
};

class ViewGroup {
public:
    ViewGroup(int32_t x, int32_t y, uint32_t w, uint32_t h) : x_(x), y_(y), width_(w), height_(h) {}
