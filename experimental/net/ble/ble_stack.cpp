...
BleConnectionState BleManager::get_state() const {
    return current_state_;
}

void BleManager::update_heart_rate(uint8_t bpm) {
    if (cached_heart_rate_ == bpm)
        return;
...
void BleManager::update_battery_level(uint8_t level) {
    if (cached_battery_level_ == level)
        return;
...
void BleManager::daemon_task() {
    while (true) {
        ...
        case 0x04: { // EVENT_NOTIFICATION_RECEIVED (手机通知推送, TLV 格式)
            ...
            const aurora::Notification n =
                aurora::BleNotificationParser::parse(event.payload, static_cast<uint8_t>(sizeof(event.payload)), tick);
