...
public:
    int read(char* buf, int len, int offset, void* /*priv*/) override {
        ...
        append_str("----------------------\n");
        append_str("MemTotal:  ");
        append_num(total_mem);
        append_str(" Bytes\n");
        append_str("MemFree:   ");
        append_num(free_mem);
        append_str(" Bytes\n");
        append_str("Defrag:    ");
        append_num(Metrics::get_heap_defrags());
        append_str(" times\n");
...
public:
    int read(char* buf, int len, int offset, void* /*priv*/) override {
        ...
        for (int i = 0; i < count; i++) {
            ...
            // 状态解析
            if (tcb->scheduler.state == TaskState::Running)
                append_str("RUN\t");
            else if (tcb->scheduler.state == TaskState::Ready)
                append_str("RDY\t");
            else if (tcb->scheduler.state == TaskState::Sleeping)
                append_str("SLP\t");
            else if (tcb->scheduler.state == TaskState::Blocked_On_Notify)
                append_str("BNT\t");
            else if (tcb->scheduler.state == TaskState::Suspended)
                append_str("SUS\t");
            else if (tcb->scheduler.state == TaskState::Terminated)
                append_str("TRM\t");
            else
                append_str("UNK\t");
...
public:
    int read(char* buf, int len, int offset, void* /*priv*/) override {
        ...
        append_str("us p99=");
        append_num(Metrics::get_recorder(METRIC_HEAP_64B).get_p99_us());
        append_str("us count=");
...
public:
    int read(char* buf, int len, int offset, void* /*priv*/) override {
        ...
        append_str("sleep_ratio ");
        append_num(Metrics::get_power_profiler().get_sleep_ratio());
        append_str("% sleep_count ");
        append_num(Metrics::get_power_profiler().get_sleep_count());
...
public:
    int read(char* buf, int len, int offset, void* /*priv*/) override {
        ...
        append_str("udp_drops ");
        append_num(Metrics::get_net_drops());
        append_str("\n");
        buf[pos] = '\0';
        return pos;
    }
...
public:
    int read(char* buf, int len, int offset, void* /*priv*/) override {
        ...
        append_str("registers ");
        append_num(Metrics::get_softbus_registers());
        append_str("\n");
        buf[pos] = '\0';
        return pos;
    }
    ...
    int read(char* buf, int len, int offset, void* /*priv*/) override {
        ...
        for (int i = 0; i < count; i++) {
            ...
            for (int s = 0; s < auroraos::kernel::MAX_CSPACE_SLOTS; s++) {
                ...
                switch (cap.type) {
                case auroraos::kernel::CapType::Endpoint:
                    append_str("Endpoint");
                    break;
                case auroraos::kernel::CapType::Thread:
                    append_str("Thread");
                    break;
