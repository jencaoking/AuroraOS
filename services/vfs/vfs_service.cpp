...
void VfsServer::handle_open(const VfsRequest& req, VfsReply& reply) {
    ...
    for (const char* p = path; *p; p++, path_len++) {
        ...
        if (path_len >= 63) { reply.status = -1; return; }
    }
    if (has_traversal) { reply.status = -1; return; }
    ...
    if (!target) { reply.status = -1; return; }
...
void VfsServer::handle_read(const VfsRequest& req, VfsReply& reply) {
    ...
    if (fd < 0 || fd >= MAX_OPEN_FILES || !fd_table_[fd].used) { reply.status = -1; return; }
    if (len < 0 || len > sizeof(reply.read.data)) len = sizeof(reply.read.data);
...
void VfsServer::handle_write(const VfsRequest& req, VfsReply& reply) {
    ...
    if (fd < 0 || fd >= MAX_OPEN_FILES || !fd_table_[fd].used) { reply.status = -1; return; }
    if (len < 0 || len > sizeof(req.write.data)) len = sizeof(req.write.data);
