...
void NetServer::handle_select(const NetRequest& req, NetReply& reply) {
    ...
    for (int i = 0; i < req.select.maxfdp1; ++i) {
        if (req.select.has_readset && (req.select.readset & (1 << i)))
            FD_SET(i, &readset);
        if (req.select.has_writeset && (req.select.writeset & (1 << i)))
            FD_SET(i, &writeset);
        if (req.select.has_exceptset && (req.select.exceptset & (1 << i)))
            FD_SET(i, &exceptset);
    }
    ...
    reply.status = lwip_select(req.select.maxfdp1, req.select.has_readset ? &readset : nullptr,
                               req.select.has_writeset ? &writeset : nullptr,
                               req.select.has_exceptset ? &exceptset : nullptr, &tv);
    ...
    if (reply.status > 0) {
        ...
        for (int i = 0; i < req.select.maxfdp1; ++i) {
            if (req.select.has_readset && FD_ISSET(i, &readset))
                reply.select.readset |= (1 << i);
            if (req.select.has_writeset && FD_ISSET(i, &writeset))
                reply.select.writeset |= (1 << i);
            if (req.select.has_exceptset && FD_ISSET(i, &exceptset))
                reply.select.exceptset |= (1 << i);
        }
    }
}
...
void NetServer::process_request(const NetRequest& req, NetReply& reply) {
    ...
    switch (req.opcode) {
    case NetOpcode::Socket:
        handle_socket(req, reply);
        break;
    case NetOpcode::Connect:
        handle_connect(req, reply);
        break;
    case NetOpcode::Bind:
        handle_bind(req, reply);
        break;
    case NetOpcode::Send:
        handle_send(req, reply);
        break;
    case NetOpcode::Recv:
        handle_recv(req, reply);
        break;
    case NetOpcode::SendTo:
        handle_sendto(req, reply);
        break;
    case NetOpcode::RecvFrom:
        handle_recvfrom(req, reply);
        break;
    case NetOpcode::Fcntl:
        handle_fcntl(req, reply);
        break;
    case NetOpcode::Setsockopt:
        handle_setsockopt(req, reply);
        break;
    case NetOpcode::Getsockopt:
        handle_getsockopt(req, reply);
        break;
    case NetOpcode::Select:
        handle_select(req, reply);
        break;
    case NetOpcode::Close:
