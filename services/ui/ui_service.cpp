        ...
        if (ipc_msg.msg_type == 1) { // UI Request
            ...
            case UiOpcode::CreateWindow: {
                reply.status = WindowManager::instance().create_window(
                    ipc_msg.req.create_window.x, ipc_msg.req.create_window.y, ipc_msg.req.create_window.width,
                    ipc_msg.req.create_window.height, ipc_msg.req.create_window.title, caller_cap);
                break;
            }
            case UiOpcode::DestroyWindow: {
                bool success = WindowManager::instance().destroy_window(ipc_msg.req.destroy_window_id, caller_cap);
