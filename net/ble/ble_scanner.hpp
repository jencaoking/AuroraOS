...
struct BleDeviceFingerprint {
    ...
    bool        is_connecting() const noexcept { return is_connectable; }
    bool        has_name()       const noexcept { return name_len > 0; }
...
public:
    ...
    const BleDeviceFingerprint* process_advertisement(
        const uint8_t mac[6], BleAddrType addr_type,
        int8_t rssi, const uint8_t* ad_data, uint8_t len) noexcept;
...
private:
    ...
    void parse_ad_structures_(BleDeviceFingerprint* dev,
    ...
    static void iter_ad_structures_(const uint8_t* data, uint8_t len,
                                     void (*cb)(const AdEntry&, void*),
                                     void* user) noexcept;
};
...
inline const BleDeviceFingerprint* BleScanner::process_advertisement(
    const uint8_t mac[6], BleAddrType addr_type,
    int8_t rssi, const uint8_t* ad_data, uint8_t len) noexcept
{
    if (!mac || !ad_data || len == 0 || len > kAdPayloadMax) return nullptr;
...
inline const BleDeviceFingerprint* BleScanner::find_by_mac(
    const uint8_t mac[6]) const noexcept
{
    ...
    for (int i = 0; i < device_count_; ++i) {
        bool match = true;
        for (int j = 0; j < 6; ++j) {
            if (devices_[i].mac[j] != mac[j]) { match = false; break; }
        }
        if (match) return &devices_[i];
    }
    return nullptr;
}
...
inline const BleDeviceFingerprint* BleScanner::get_device(int index) const noexcept {
    LockGuard lock(mutex_);
    if (index < 0 || index >= device_count_) return nullptr;
    return &devices_[index];
}

inline const BleDeviceFingerprint* BleScanner::find_by_name(
    const char* name) const noexcept
{
    if (!name) return nullptr;
    LockGuard lock(mutex_);
    for (int i = 0; i < device_count_; ++i) {
        if (devices_[i].has_name() &&
            strncmp(devices_[i].name, name, sizeof(devices_[i].name)) == 0) {
            return &devices_[i];
        }
    }
    return nullptr;
...
inline int BleScanner::count_by_class(BleDeviceClass cls) const noexcept {
    ...
    int n = 0;
    for (int i = 0; i < device_count_; ++i) {
        if (devices_[i].device_class == cls) ++n;
    }
    return n;
}
...
inline int BleScanner::count_weak_security() const noexcept {
    ...
    for (int i = 0; i < device_count_; ++i) {
        ...
        if (!devices_[i].has_sc_confirm && !devices_[i].has_sc_random &&
            !devices_[i].has_oob_support) {
            ++n;
        }
    }
    return n;
...
inline BleDeviceFingerprint* BleScanner::find_or_add_(
    const uint8_t mac[6]) noexcept
{
    ...
    // Add new
    if (device_count_ >= kMaxDevices) return nullptr;

    BleDeviceFingerprint* dev = &devices_[device_count_];
    for (int j = 0; j < 6; ++j) dev->mac[j] = mac[j];
...
inline void BleScanner::parse_ad_structures_(
    BleDeviceFingerprint* dev, const uint8_t* data, uint8_t len) noexcept
{
    uint8_t pos = 0;
    while (pos + 1 < len) {
        ...
        case BleAdType::CompleteName:
            if (value_len > 0) {
                dev->name_len = (value_len < BleDeviceFingerprint::kMaxNameLen)
                              ? value_len : BleDeviceFingerprint::kMaxNameLen;
        ...
        case BleAdType::CompleteUuid16:
            ...
            if (dev->uuid16_count > 4) dev->uuid16_count = 4;
            for (uint8_t k = 0; k < dev->uuid16_count; ++k) {
                dev->uuid16_list[k] = static_cast<uint16_t>(field_data[k * 2])
                                    | (static_cast<uint16_t>(field_data[k * 2 + 1]) << 8);
            }
            break;

        case BleAdType::Appearance:
            if (value_len >= 2) {
                dev->appearance = static_cast<uint16_t>(field_data[0])
                                | (static_cast<uint16_t>(field_data[1]) << 8);
            }
            break;

        case BleAdType::TxPowerLevel:
            if (value_len >= 1) dev->tx_power_dbm = static_cast<int8_t>(field_data[0]);
            break;

        case BleAdType::ManufacturerData:
            if (value_len >= 2) {
                dev->company_id = static_cast<uint16_t>(field_data[0])
                                | (static_cast<uint16_t>(field_data[1]) << 8);
...
inline void BleScanner::update_rssi_(
    BleDeviceFingerprint* dev, int8_t rssi) noexcept
{
    ...
    for (uint8_t i = 0; i < dev->rssi_count; ++i) {
        ...
        if (dev->rssi_history[i] < mn) mn = dev->rssi_history[i];
        if (dev->rssi_history[i] > mx) mx = dev->rssi_history[i];
    }
...
inline void BleScanner::classify_device_(
    BleDeviceFingerprint* dev) noexcept
{
...
inline void BleScanner::iter_ad_structures_(
    const uint8_t* data, uint8_t len,
    void (*cb)(const AdEntry&, void*), void* user) noexcept
{
    if (!data || !cb) return;
    uint8_t pos = 0;
    while (pos + 1 < len) {
