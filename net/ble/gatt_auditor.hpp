...
static bool is_insecure(uint16_t uuid16) noexcept {
    return uuid16 == ANCS || uuid16 == ImmediateAlert || uuid16 == LinkLoss || uuid16 == CurrentTime ||
...
static const char* description(uint16_t uuid16) noexcept {
    switch (uuid16) {
    case ANCS:
        return "ANCS: known auth bypass CVEs";
    case ImmediateAlert:
        return "Immediate Alert: DoS via write flooding";
    case LinkLoss:
        return "Link Loss: persistent notification spam";
    case CurrentTime:
        return "Current Time: spoofable via write without auth";
    case HumanInterfaceDev:
        return "HID: potential keystroke injection";
    default:
        return "";
    }
}
...
public:
    ...
    static const char* severity_name(GattAuditSeverity s) noexcept {
        switch (s) {
        case GattAuditSeverity::None:
            return "None";
        case GattAuditSeverity::Info:
            return "Info";
        case GattAuditSeverity::Warning:
            return "Warning";
        case GattAuditSeverity::Critical:
            return "Critical";
        default:
            return "?";
        }
    ...
    static const char* finding_type_name(GattFindingType t) noexcept {
        ...
        case GattFindingType::OpenWriteNoAuth:
            return "OpenWriteNoAuth";
        case GattFindingType::OpenReadSensitive:
            return "OpenReadSensitive";
        ...
        case GattFindingType::JustWorksPairing:
            return "JustWorksPairing";
        case GattFindingType::NoOobSupport:
            return "NoOobSupport";
        case GattFindingType::KnownInsecureService:
            return "KnownInsecureService";
        case GattFindingType::UnauthenticatedWrite:
            return "UnauthenticatedWrite";
        case GattFindingType::InsufficientKeySize:
...
inline GattAuditor::AuditedDevice* GattAuditor::register_device(const uint8_t mac[6]) noexcept {
    if (!mac)
        return nullptr;
    ...
    if (audited_count_ >= kMaxAuditedDevices)
        return nullptr;
...
inline bool GattAuditor::add_characteristic(const uint8_t mac[6], const GattCharInfo& ch) noexcept {
    if (!mac)
        return false;
    LockGuard lock(mutex_);
    AuditedDevice* dev = find_device_(mac);
    if (!dev || dev->char_count >= kMaxCharsPerDev)
        return false;
...
inline int GattAuditor::audit_all() noexcept {
    ...
    for (uint8_t i = 0; i < audited_count_; ++i) {
        if (!audited_devices_[i].connected)
            continue;
...
inline void GattAuditor::audit_char_(const AuditedDevice& dev, const GattCharInfo& ch) noexcept {
    GattFinding f{};
    f.timestamp = tick_count;
    for (int i = 0; i < 6; ++i)
        f.device_mac[i] = dev.mac[i];
...
inline const GattFinding* GattAuditor::get_finding(int index) const noexcept {
    LockGuard lock(mutex_);
    if (index < 0 || index >= finding_count_)
        return nullptr;
...
inline const GattAuditor::AuditedDevice* GattAuditor::get_audited_device(int index) const noexcept {
    LockGuard lock(mutex_);
    if (index < 0 || index >= audited_count_)
        return nullptr;
...
inline GattAuditor::AuditedDevice* GattAuditor::find_device_(const uint8_t mac[6]) noexcept {
    for (uint8_t i = 0; i < audited_count_; ++i) {
        bool match = true;
        for (int j = 0; j < 6; ++j) {
            if (audited_devices_[i].mac[j] != mac[j]) {
                match = false;
                break;
            }
        }
        if (match)
            return &audited_devices_[i];
    }
    return nullptr;
}
