...
public:
    ...
    void on_connect(const uint8_t mac[6], const BleConnParams& params,
                    int8_t rssi, bool le_secure) noexcept;
    ...
    void on_pairing_complete(const uint8_t mac[6],
                              BlePairingMethod method,
                              uint8_t security_level,
                              bool le_secure,
                              bool oob) noexcept;

    // Called when connection parameters update.
    void on_params_update(const uint8_t mac[6],
                           const BleConnParams& new_params) noexcept;

    // Called when RSSI is read (from periodic RSSI reads or scan).
    void on_rssi_update(const uint8_t mac[6],
                          int8_t rssi, int8_t tx_power) noexcept;
    ...
    static const char* alert_type_name(MitmAlertType t) noexcept {
        switch (t) {
            case MitmAlertType::PairingDowngrade:         return "PairingDowngrade";
            case MitmAlertType::ParamIntervalAnomaly:     return "ParamIntervalAnomaly";
            case MitmAlertType::ParamLatencyAnomaly:      return "ParamLatencyAnomaly";
            case MitmAlertType::RssiParadox:             return "RssiParadox";
            case MitmAlertType::DisconnectStorm:          return "DisconnectStorm";
            case MitmAlertType::KeyRenegotiationFreq:    return "KeyRenegotiationFreq";
            case MitmAlertType::AddressSpoofing:         return "AddressSpoofing";
            case MitmAlertType::SupervisionTimeoutLong:   return "SupervisionTimeoutLong";
            case MitmAlertType::HopIncrementAnomaly:     return "HopIncrementAnomaly";
            case MitmAlertType::SecurityLevelDowngrade:   return "SecurityLevelDowngrade";
    ...
    static const char* severity_name(MitmSeverity s) noexcept {
        switch (s) {
            case MitmSeverity::Info:     return "Info";
            case MitmSeverity::Warning:  return "Warning";
            case MitmSeverity::Alert:    return "Alert";
            case MitmSeverity::Critical: return "Critical";
            default:                     return "?";
        }
    }
...
private:
    ...
    void detect_param_anomaly_(const MitmSession& s,
                                const BleConnParams& new_params) noexcept;
...
inline void BleMitmDetector::on_connect(
    const uint8_t mac[6], const BleConnParams& params,
    int8_t rssi, bool le_secure) noexcept
{
    if (!mac) return;
    ...
    if (!s) {
        if (session_count_ >= kMaxSessions) return;
        s = &sessions_[session_count_];
        for (int i = 0; i < 6; ++i) s->mac[i] = mac[i];
        s->active                 = true;
...
inline void BleMitmDetector::on_disconnect(
    const uint8_t mac[6], uint8_t reason) noexcept
{
    if (!mac) return;
    LockGuard lock(mutex_);
...
inline void BleMitmDetector::on_pairing_complete(
    const uint8_t mac[6], BlePairingMethod method,
    uint8_t security_level, bool le_secure, bool oob) noexcept
{
    if (!mac) return;
    LockGuard lock(mutex_);
    ...
    if (s->initial_pairing_method == BlePairingMethod::JustWorks &&
        method != BlePairingMethod::JustWorks) {
...
inline void BleMitmDetector::on_params_update(
    const uint8_t mac[6], const BleConnParams& new_params) noexcept
{
    if (!mac) return;
    LockGuard lock(mutex_);
...
inline void BleMitmDetector::on_rssi_update(
    const uint8_t mac[6], int8_t rssi, int8_t tx_power) noexcept
{
    if (!mac) return;
    LockGuard lock(mutex_);
...
inline void BleMitmDetector::detect_pairing_downgrade_(
    const MitmSession& s) noexcept
{
    ...
    if (s.initial_pairing_method >= BlePairingMethod::PasskeyEntry &&
        ...
        for (int i = 0; i < 6; ++i) a.device_mac[i] = s.mac[i];
        ...
        for (int i = 0; i < 71 && desc[i]; ++i) a.description[i] = desc[i];
        a.description[71] = '\0';
        add_alert_(a);
    }
...
inline void BleMitmDetector::detect_param_anomaly_(
    const MitmSession& s, const BleConnParams& new_params) noexcept
{
    ...
    if (s.current_params.latency > 0 &&
        new_params.latency > s.current_params.latency * 5) {
        MitmAlert a{};
        a.timestamp   = tick_count;
        for (int i = 0; i < 6; ++i) a.device_mac[i] = s.mac[i];
...
inline void BleMitmDetector::detect_rssi_paradox_(
    const MitmSession& s, int8_t rssi, int8_t tx_power) noexcept
{
    ...
    if (rssi_delta > rssi_paradox_threshold_ &&
        tx_power == s.last_tx_power && s.last_tx_power != 0) {
        MitmAlert a{};
        a.timestamp   = tick_count;
        for (int i = 0; i < 6; ++i) a.device_mac[i] = s.mac[i];
...
inline void BleMitmDetector::detect_disconnect_storm_(
    const MitmSession& s) noexcept
{
...
inline void BleMitmDetector::detect_supervision_anomaly_(
    const MitmSession& s) noexcept
{
...
inline MitmSession* BleMitmDetector::find_session_(
    const uint8_t mac[6]) noexcept
{
    for (int i = 0; i < session_count_; ++i) {
        bool match = true;
        for (int j = 0; j < 6; ++j) {
            if (sessions_[i].mac[j] != mac[j]) { match = false; break; }
        }
        if (match) return &sessions_[i];
    }
    return nullptr;
}
