...
public:
    ...
    static const char* event_type_name(BleIdsEventType t) noexcept {
        switch (t) {
        case BleIdsEventType::ScanStorm:
            return "ScanStorm";
        case BleIdsEventType::ConnectFlood:
            return "ConnectFlood";
        case BleIdsEventType::GattDiscoveryStorm:
            return "GattDiscoveryStorm";
        case BleIdsEventType::WeakSecurity:
            return "WeakSecurity";
        case BleIdsEventType::MacSpoofing:
            return "MacSpoofing";
        case BleIdsEventType::KnownVulnerableSvc:
            return "KnownVulnerableSvc";
        case BleIdsEventType::PairingDowngrade:
            return "PairingDowngrade";
        case BleIdsEventType::UnauthorizedWrite:
            return "UnauthorizedWrite";
        case BleIdsEventType::RssiSpoofing:
            return "RssiSpoofing";
        case BleIdsEventType::ExcessiveMtu:
            return "ExcessiveMtu";
        default:
            return "Unknown";
        }
...
inline void BleIds::feed_advertisement(const uint8_t mac[6], int8_t /*rssi*/, uint8_t ad_flags) noexcept {
    if (!mac)
        return;
    LockGuard lock(mutex_);
...
inline void BleIds::feed_gatt_finding(const GattFinding& finding) noexcept {
    ...
    for (int i = 0; i < 6; ++i)
        evt.mac[i] = finding.device_mac[i];
...
inline void BleIds::feed_connection_event(const uint8_t mac[6], bool is_connect, uint8_t /*reason*/) noexcept {
    if (!mac)
        return;
    LockGuard lock(mutex_);
...
inline void BleIds::evaluate_rules_() noexcept {
    ...
    for (int r = 0; r < rule_count_; ++r) {
        ...
        for (int e = start; e < event_count_; ++e) {
            ...
            if ((now - evt.timestamp) > rule.window_ms)
                continue;
            if (evt.event_type != rule.event_type)
                continue;
            if (evt.severity < rule.min_severity)
                continue;
            ++match_count;
        }
        ...
        if (match_count >= rule.threshold_count) {
            ...
            if (alert_count_ < kMaxAlerts) {
                ...
                a.severity = (match_count >= rule.threshold_count * 3)   ? 90u
                             : (match_count >= rule.threshold_count * 2) ? 70u
                                                                         : 50u;
...
inline int BleIds::procfs_read(char* buf, int len, int /*offset*/, void* /*priv*/) {
    ...
    append("Total Events:  ");
    append_int(ids.event_count());
    append("\n");
    append("Total Alerts:  ");
    append_int(ids.alert_count());
    append("\n");
    append("Active Rules:  ");
    append_int(ids.rule_count());
    append("\n\n");
    ...
    for (int i = alert_start; i < alert_total && pos < len - 80; ++i) {
        ...
        append_int(a->timestamp / 1000);
        append("s | ");
        append(event_type_name(a->event_type));
        append(" | S=");
        append_int(a->severity);
        append(" | cnt=");
        append_int(a->event_count);
        append("\n");
    }
