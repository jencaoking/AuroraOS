...
public:
    void set_engine(class WirelessIds* engine) {
        engine_ = engine;
    }
...
public:
    ...
    void submit_event(WirelessEventType type, const uint8_t* bssid, const uint8_t* station, int8_t rssi,
                      uint16_t param) {
        WirelessEvent ev{};
    ...
    // 快速提交（无 Station）
    void submit_event_simple(WirelessEventType type, const uint8_t* bssid, int8_t rssi, uint16_t param) {
    ...
    void scan_beacons() {
        ...
        for (int i = 0; i < beacon.get_ap_count(); ++i) {
            ...
            if (BeaconAnalyzer::is_weak_encryption(ap->encryption)) {
                submit_event_simple(WirelessEventType::WeakEncryption, ap->bssid, ap->rssi_dbm, ap->channel);
            }
        }
        ...
        if (hidden > 0) {
            for (int i = 0; i < beacon.get_ap_count(); ++i) {
                const ApInfo* ap = beacon.get_ap(i);
                if (ap && ap->is_hidden) {
                    submit_event_simple(WirelessEventType::HiddenSsid, ap->bssid, ap->rssi_dbm, ap->channel);
                }
            }
        }
        ...
        for (int i = 0; i < rogue_count; ++i) {
            submit_event(WirelessEventType::EvilTwin, rogue_alerts[i].bssid_b, rogue_alerts[i].bssid_a, 0,
                         rogue_alerts[i].channel_b);
        }
    }
    ...
    void scan_deauths() {
        ...
        for (int i = 0; i < recent; ++i) {
            submit_event_simple(WirelessEventType::DeauthFlood, &recent_bssids[i * 6], 0, 0);
        }
    }

    // ---- 告警管理 ----

    int get_alert_count() const {
        return alert_count_;
    }
    ...
    uint32_t get_total_alerts() const {
        return alert_generated_;
    }
    ...
    static const char* event_type_name(WirelessEventType t) {
        switch (t) {
        case WirelessEventType::DeauthFlood:
            return "deauth_flood";
        case WirelessEventType::EvilTwin:
            return "evil_twin";
        case WirelessEventType::WeakEncryption:
            return "weak_encryption";
        case WirelessEventType::HiddenSsid:
            return "hidden_ssid";
        case WirelessEventType::SpoofedFrame:
            return "spoofed_frame";
        case WirelessEventType::HandshakeCapture:
            return "handshake_capture";
        case WirelessEventType::ChannelHop:
            return "channel_hop";
        case WirelessEventType::WpsDetected:
            return "wps_detected";
        case WirelessEventType::RogueProbeResp:
            return "rogue_probe_resp";
        case WirelessEventType::UnknownFrameType:
...
private:
    ...
    void evaluate_rules_(const WirelessEvent& ev) {
        for (int i = 0; i < kMaxRules; ++i) {
            ...
            if (!rule.enabled)
                continue;
            if (rule.event_type != static_cast<WirelessEventType>(ev.event_type))
                continue;
            if (ev.severity < rule.min_severity)
                continue;
            ...
            if (rule.threshold_count > 0) {
                ...
                for (int j = 0; j < limit; ++j) {
                    ...
                    if (hist.timestamp < window_start)
                        break;
                    if (hist.event_type == ev.event_type)
                        ++match_count;
                }
            ...
            for (int j = 0; j < 6 && dpos < 127 - 3; ++j) {
                ...
                char l = b & 0xF;
                if (dpos < 127)
                    alert.description[dpos++] = h < 10 ? '0' + h : 'A' + h - 10;
                if (dpos < 127)
                    alert.description[dpos++] = l < 10 ? '0' + l : 'A' + l - 10;
                if (j < 5 && dpos < 127)
                    alert.description[dpos++] = ':';
            }
            ...
            // 执行规则动作
            if (rule.action >= 2) {
                SecurityMonitor::instance().report_firewall_anomaly(alert.description);
            }
        }
    }
...
inline int WirelessIdsNode::read(char* buf, int len, int offset, void* /*priv*/) {
    ...
    app_s("Total Events: ");
    app_u(engine_->get_total_events());
    app_s("\n");
    app_s("Total Alerts: ");
    app_u(engine_->get_total_alerts());
    app_s("\n");
    app_s("Active Rules: ");
    app_u(engine_->get_rule_count());
    app_s("\n");
    app_s("\n--- Recent Alerts ---\n");
