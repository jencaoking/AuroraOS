...
static bool rtl_reg_write_mask(UsbDevice& dev, uint16_t reg, uint32_t mask, uint32_t set) {
    uint32_t val;
    if (!rtl_reg_read(dev, reg, &val))
        return false;
...
bool Rtl8187lMonitor::init() {
    ...
    if (dev.vendor_id != 0x0BDA || (dev.product_id != 0x8187 && dev.product_id != 0x8189)) {
        state_ = DriverState::Error;
        return false;
    }
    ...
    if (rtl_reg_read(dev, Rtl8187lReg::MAC_ADDR, &mac_lo) && rtl_reg_read(dev, Rtl8187lReg::MAC_ADDR + 4, &mac_hi)) {
...
bool Rtl8187lMonitor::enter_monitor_mode() {
    ...
    uint32_t rx_conf = RtlRxConf::RX_MONITOR | RtlRxConf::RX_CTRL_FRAME | RtlRxConf::RX_BAD_FCS |
                       RtlRxConf::RX_ALL_MULTICAST | RtlRxConf::RX_BROADCAST | RtlRxConf::RX_DATA_FRAME;
    if (!rtl_reg_write(dev, Rtl8187lReg::REG_RX_CONF, rx_conf))
        return false;
...
bool Rtl8187lMonitor::set_channel(uint16_t frequency_mhz) {
    ...
    if (!rtl_reg_write(dev, Rtl8187lReg::REG_CHANNEL, ch))
        return false;
...
int Rtl8187lMonitor::capture_frame(CapturedFrame& out_frame) {
    ...
    if (!rx_ok || frame_len <= 0 || frame_len > 2346)
        return 0;
