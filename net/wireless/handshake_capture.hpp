...
public:
    ...
    int process_eapol(const CapturedFrame& frame) {
        ...
        if (remain < static_cast<int>(sizeof(WifiMacHeader)) + 4)
            return 0;
        ...
        if (ft != FrameType::Data && ft != FrameType::DataQos)
            return 0;

        // 跳过 802.11 头 → LLC/SNAP → EAPOL
        int l2_offset = sizeof(WifiMacHeader);
        // QoS Data 有额外的 QoS Control 字段（2 字节）
        if (ft == FrameType::DataQos)
            l2_offset += 2;

        if (l2_offset + 14 > remain)
            return 0; // LLC(8) + SNAP(6) 最小值
    ...
    const HandshakeSession* get_session(int index) const {
        if (index < 0 || index >= session_count_)
            return nullptr;
    ...
    int export_hashcat_format(char* out_buf, int max_len) const {
        ...
        for (int i = 0; i < session_count_; ++i) {
            ...
            if (s.complete) {
                ...
                // ESSID (hex-encoded)
                for (int j = 0; j < 32 && s.ssid[j]; ++j)
                    app_hex(static_cast<uint8_t>(s.ssid[j]));
                app_char('*');
...
private:
    ...
    int process_eapol_frame_(const CapturedFrame& frame, const WifiMacHeader* mac, const uint8_t* eapol, int len) {
        if (len < 4)
            return 0;
        ...
        // 判定消息类型
        HandshakeMessage msg = classify_handshake_msg_(has_key_ack, has_key_mic, has_install, has_secure);
        if (msg == HandshakeMessage::None)
            return 0;
        ...
        if (has_key_mic && body_len >= 97) {
            if (msg == HandshakeMessage::M2) {
                for (int i = 0; i < 16; ++i)
                    sess->mic_m2[i] = key_data[81 + i];
            }
            if (msg == HandshakeMessage::M3) {
                for (int i = 0; i < 16; ++i)
                    sess->mic_m3[i] = key_data[81 + i];
            }
        }
        ...
        // 检查握手是否完整
        if (sess->eapol_m1_len > 0 && sess->eapol_m2_len > 0 && sess->eapol_m3_len > 0 && sess->eapol_m4_len > 0) {
            sess->complete = true;
            return 2;
        }
    ...
    static HandshakeMessage classify_handshake_msg_(bool key_ack, bool key_mic, bool install, bool secure) {
        // M1: KeyAck=1, KeyMic=0, Install=0, Secure=0
        if (key_ack && !key_mic && !install && !secure)
            return HandshakeMessage::M1;
        // M2: KeyAck=0, KeyMic=1, Install=0, Secure=0
        if (!key_ack && key_mic && !install && !secure)
            return HandshakeMessage::M2;
        // M3: KeyAck=1, KeyMic=1, Install=1, Secure=0|1
        if (key_ack && key_mic && install)
            return HandshakeMessage::M3;
        // M4: KeyAck=0, KeyMic=1, Install=0, Secure=1
        if (!key_ack && key_mic && !install && secure)
            return HandshakeMessage::M4;
    ...
    HandshakeSession* find_or_create_session_(const uint8_t* sta, const uint8_t* ap) {
        // 查找已存在会话
        for (int i = 0; i < session_count_; ++i) {
            if (mac_equal_(sessions_[i].sta_mac, sta) && mac_equal_(sessions_[i].ap_mac, ap)) {
                return &sessions_[i];
            }
        }
        ...
        int write = 0;
        for (int i = 0; i < session_count_; ++i) {
            if (now - sessions_[i].last_seen_tick < 60000 && !sessions_[i].complete) { // 60s 超时
                if (write != i)
                    sessions_[write] = sessions_[i];
                ++write;
            }
        }
    ...
    void store_eapol_frame_(HandshakeSession* sess, HandshakeMessage msg, const uint8_t* eapol, int len) {
        uint8_t* dst = nullptr;
        uint16_t* dst_len = nullptr;
        int max_store = 0;

        switch (msg) {
        case HandshakeMessage::M1:
            dst = sess->eapol_m1;
            dst_len = &sess->eapol_m1_len;
            max_store = 256;
            break;
        case HandshakeMessage::M2:
            dst = sess->eapol_m2;
            dst_len = &sess->eapol_m2_len;
            max_store = 256;
            break;
        case HandshakeMessage::M3:
            dst = sess->eapol_m3;
            dst_len = &sess->eapol_m3_len;
            max_store = 256;
            break;
        case HandshakeMessage::M4:
            dst = sess->eapol_m4;
            dst_len = &sess->eapol_m4_len;
            max_store = 256;
            break;
        default:
            return;
    ...
    void extract_pmkid_(const uint8_t* key_data, int body_len, HandshakeSession* sess) {
        ...
        if (kd_available < static_cast<int>(kd_len))
            return;
