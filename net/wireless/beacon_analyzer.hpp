...
public:
    ...
    // 获取已知 AP 数量
    int get_ap_count() const {
        return ap_count_;
    }

    // 按索引获取 AP 信息
    const ApInfo* get_ap(int index) const {
        if (index < 0 || index >= ap_count_)
            return nullptr;
    ...
    int detect_rogue_aps(RogueApAlert* out_alerts, int max_alerts) const {
        ...
        for (int i = 0; i < ap_count_ && alert_count < max_alerts; ++i) {
            if (ap_table_[i].ssid_len == 0)
                continue;
            for (int j = i + 1; j < ap_count_ && alert_count < max_alerts; ++j) {
                if (ap_table_[j].ssid_len == 0)
                    continue;
    ...
    static const char* encryption_name(EncryptionType t) {
        ...
        case EncryptionType::WpaPskTkip:
            return "WPA-TKIP";
        case EncryptionType::WpaPskCcmp:
            return "WPA-AES";
        case EncryptionType::Wpa2PskTkip:
            return "WPA2-TKIP";
        case EncryptionType::Wpa2PskCcmp:
            return "WPA2-AES";
        case EncryptionType::Wpa2Enterprise:
            return "WPA2-Enterprise";
        case EncryptionType::Wpa3Sae:
            return "WPA3-SAE";
        case EncryptionType::Wpa3Enterprise:
            return "WPA3-Enterprise";
        case EncryptionType::Mixed:
            return "Mixed";
        default:
            return "Unknown";
        }
    ...
    static bool is_weak_encryption(EncryptionType t) {
        return t == EncryptionType::Open || t == EncryptionType::Wep || t == EncryptionType::WpaPskTkip ||
...
private:
    ...
    // 解析 Beacon 帧体
    bool parse_beacon_body_(const CapturedFrame& frame, const WifiMacHeader* mac, const uint8_t* body, int body_len) {
        ApInfo info{};
        for (int i = 0; i < 6; ++i)
            info.bssid[i] = mac->addr3[i];
    ...
    void parse_rsn_ie_(const uint8_t* data, uint8_t len, ApInfo& info) {
        ...
        for (int i = 0; i < static_cast<int>(akm_count) && pos + 4 <= len; ++i) {
            uint32_t oui =
                (static_cast<uint32_t>(data[pos]) << 16) | (static_cast<uint32_t>(data[pos + 1]) << 8) | data[pos + 2];
    ...
    static void format_oui_(const uint8_t* mac, char* out) {
        ...
        out[0] = hex(mac[0] >> 4);
        out[1] = hex(mac[0] & 0x0F);
        out[2] = ':';
        out[3] = hex(mac[1] >> 4);
        out[4] = hex(mac[1] & 0x0F);
        out[5] = ':';
        out[6] = hex(mac[2] >> 4);
        out[7] = hex(mac[2] & 0x0F);
    }
