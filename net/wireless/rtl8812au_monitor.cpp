...
static bool rtl_read8(UsbDevice& dev, uint16_t reg, uint8_t* val) {
    uint32_t v32;
    if (!UsbHost::instance().read_register(dev, reg, &v32)) return false;
...
bool Rtl8812auMonitor::init() {
    ...
    if (dev.vendor_id != 0x0BDA ||
        (dev.product_id != 0x8812 && dev.product_id != 0x881A &&
         dev.product_id != 0x881B)) {
        state_ = DriverState::Error;
        return false;
    }
...
bool Rtl8812auMonitor::enter_monitor_mode() {
    ...
    rtl_write32(dev, Rtl8812auReg::REG_CR,
                RtlCr::CR_RE | RtlCr::CR_HCI_RX);
...
bool Rtl8812auMonitor::leave_monitor_mode() {
    ...
    if (UsbHost::instance().enumerate_device(dev)) {
        ...
        rtl_write32(dev, Rtl8812auReg::REG_RCR, RtlRcr::NORMAL);
        rtl_write32(dev, Rtl8812auReg::REG_CR,
                    RtlCr::CR_TE | RtlCr::CR_RE |
                    RtlCr::CR_HCI_RX | RtlCr::CR_HCI_TX);
    }
...
int Rtl8812auMonitor::capture_frame(CapturedFrame& out_frame) {
    ...
    if (rx_len < RX_DESC_SIZE + 14) return 0;  // min: desc + 802.11 header

    uint32_t dword0 = buf[0] | (static_cast<uint32_t>(buf[1]) << 8)
                    | (static_cast<uint32_t>(buf[2]) << 16)
                    | (static_cast<uint32_t>(buf[3]) << 24);
    uint16_t frame_len = static_cast<uint16_t>(dword0 & 0x3FFF);

    if (frame_len == 0 || frame_len > 2346 ||
        RX_DESC_SIZE + frame_len > static_cast<uint16_t>(rx_len)) {
        return 0;
    }
...
bool Rtl8812auMonitor::upload_firmware(const uint8_t* fw_data, size_t fw_size) {
    ...
    for (size_t offset = 0; offset < fw_size; offset += CHUNK) {
        ...
        UsbTransferResult r = UsbHost::instance().bulk_out(
            dev, 2, fw_data + offset, static_cast<int>(chunk));
    ...
    uint32_t himr;
    if (!rtl_read32(dev, Rtl8812auReg::REG_HIMR, &himr)) return false;
    // Bit 0 = MCU ready interrupt
    if (!(himr & 0x00000001)) return false;
