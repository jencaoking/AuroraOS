...
public:
    ...
    int probe_vulnerabilities(uint32_t ip, uint16_t port, const char* service_name,
        ...
        for (int i = 0; i < cve_count_; ++i) {
            ...
            if (service_name && service_name[0] != '\0') {
                if (!match_pattern_(service_name, sig.affected_service)) continue;
            }
            if (sig.default_port != 0 && sig.default_port != port) continue;
    ...
    int probe_by_version(uint32_t ip, uint16_t port,
        ...
        for (int i = 0; i < cve_count_; ++i) {
            if (count >= max_results) break;
            const CveSignature& sig = cve_signatures_[i];
            if (!service_name || !match_pattern_(service_name, sig.affected_service)) continue;
            if (version && version[0] != '\0' && sig.affected_version[0] != '\0') {
                if (!version_in_range_(version, sig.affected_version)) continue;
            }
            out_results[count] = probe_cve(ip, port, sig);
            ++count;
            yield_cpu_();
...
private:
    ...
    static bool match_pattern_(const char* haystack, const char* needle) {
        ...
        int hlen = 0; while (haystack[hlen]) ++hlen;
        int nlen = 0; while (needle[nlen]) ++nlen;
        if (nlen > hlen) return false;
        for (int i = 0; i <= hlen - nlen; ++i) {
            bool match = true;
            for (int j = 0; j < nlen; ++j) {
                char hc = haystack[i + j]; if (hc >= 'A' && hc <= 'Z') hc += 32;
                char nc = needle[j];       if (nc >= 'A' && nc <= 'Z') nc += 32;
    ...
    static bool version_in_range_(const char* version, const char* range) {
        ...
        int rlen = 0; while (range[rlen]) ++rlen;
