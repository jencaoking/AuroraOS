...
private:
    ...
    bool identify_by_port_(ServiceInfo& info) {
        for (int i = 0; i < fingerprint_count_; ++i) {
            if (fingerprints_[i].default_port == info.port) {
                copy_str_(info.service, fingerprints_[i].service_name,
                          sizeof(info.service));
                info.identified = true;
                return true;
            }
    ...
    static bool match_pattern_(const char* banner, const char* pattern) {
        ...
        int blen = 0; while (banner[blen]) ++blen;
        int plen = 0; while (pattern[plen]) ++plen;
        if (plen > blen) return false;
        for (int i = 0; i <= blen - plen; ++i) {
            bool match = true;
            for (int j = 0; j < plen; ++j) {
                char bc = banner[i + j]; if (bc >= 'A' && bc <= 'Z') bc += 32;
                char pc = pattern[j];    if (pc >= 'A' && pc <= 'Z') pc += 32;
                if (bc != pc) { match = false; break; }
            }
    ...
    static void extract_http_server_(const char* buf, int len, ServiceInfo& info) {
        for (int i = 0; i < len - 8; ++i) {
            if ((buf[i] == 'S' || buf[i] == 's') &&
                ...
                for (int j = 0; j < copy_len; ++j) info.version[j] = buf[start + j];
                ...
                for (int j = 0; j < name_len; ++j) info.service[j] = info.version[j];
        ...
        for (int i = 0; i < len - 2; ++i) {
            if (buf[i] >= '0' && buf[i] <= '9') {
                ...
                while (start > 0 && buf[start-1] != ' ' && buf[start-1] != '\n' &&
                       buf[start-1] != '\r') --start;
                int end = i;
                while (end < len && buf[end] != ' ' && buf[end] != '\r' &&
                       buf[end] != '\n') ++end;
                int copy_len = (end - start < static_cast<int>(sizeof(info.version) - 1))
                               ? (end - start) : static_cast<int>(sizeof(info.version) - 1);
                for (int j = 0; j < copy_len; ++j) info.version[j] = buf[start + j];
