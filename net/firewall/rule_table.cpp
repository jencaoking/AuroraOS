...
FwAction RuleTable::match(const uint8_t* packet, int len, const char* interface) const {
    ...
    if (eth_type != 0x0800)
        return FwAction::ACCEPT; // Only process IPv4 for now
    if (len < 34)
        return FwAction::ACCEPT;
    ...
    for (int i = 0; i < MAX_RULES; i++) {
        ...
        if (r.match_interface && strncmp(r.interface, interface, sizeof(r.interface)) != 0)
            continue;
        if (r.match_src_ip && r.src_ip != src_ip)
            continue;
        if (r.match_dst_ip && r.dst_ip != dst_ip)
            continue;
        if (r.match_protocol && r.protocol != protocol)
            continue;
        if (r.match_src_port && r.src_port != src_port)
            continue;
        if (r.match_dst_port && r.dst_port != dst_port)
            continue;
