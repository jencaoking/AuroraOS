...
public:
    ...
    static bool match_filter(const uint8_t* pkt, int len, const BpfFilter& f) {
        // 解析以太网头 (14 字节)
        if (len < 14)
            return !any_enabled_(f); // 太短无法解析 → 无过滤规则则放行
        ...
        if (eth_type != 0x0800) {
            ...
            if (f.ip_src.enabled || f.ip_dst.enabled || f.proto_enabled || f.port_src.enabled || f.port_dst.enabled ||
                f.tcp_flags_enabled) {
                l3_pass = false;
            }
        } else {
...
private:
    ...
    static bool any_enabled_(const BpfFilter& f) {
        return f.mac_src.enabled || f.mac_dst.enabled || f.ip_src.enabled || f.ip_dst.enabled || f.proto_enabled ||
               f.port_src.enabled || f.port_dst.enabled || f.tcp_flags_enabled;
    }
    ...
    static bool compound_eval_(bool mac, bool ip, bool l4, const BpfFilter& f) {
        if (f.filter_or) {
            ...
            if (f.mac_src.enabled || f.mac_dst.enabled) {
                if (mac)
                    return true;
                any_active = true;
            }
            if (f.ip_src.enabled || f.ip_dst.enabled || f.proto_enabled) {
                if (ip)
                    return true;
                any_active = true;
            }
    ...
    static bool match_mac_(const uint8_t* pkt, const BpfFilter& f) {
        ...
        if (f.mac_src.enabled) {
            src_ok = true;
            for (int i = 0; i < 6; ++i)
                if (pkt[6 + i] != f.mac_src.addr[i]) {
                    src_ok = false;
                    break;
                }
        }
        if (f.mac_dst.enabled) {
            dst_ok = (pkt[0] == 0xFF && pkt[1] == 0xFF && pkt[2] == 0xFF && pkt[3] == 0xFF && pkt[4] == 0xFF &&
                      pkt[5] == 0xFF);
            // broadcast 总是匹配 — 否则逐字节对比
            if (!dst_ok) {
                dst_ok = true;
                for (int i = 0; i < 6; ++i)
                    if (pkt[i] != f.mac_dst.addr[i]) {
                        dst_ok = false;
                        break;
                    }
            }
    ...
    static bool match_transport_(const uint8_t* pkt, int len, uint8_t proto, int ip_hdr_sz, const BpfFilter& f) {
        if (!f.port_src.enabled && !f.port_dst.enabled && !f.tcp_flags_enabled)
            return true;
