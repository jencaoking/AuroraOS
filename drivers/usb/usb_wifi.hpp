...
public:
    ...
    // USB 控制传输（读寄存器）
    virtual bool read_register(uint8_t request, uint16_t value, uint16_t index, uint8_t* data, uint16_t len) = 0;

    // USB 控制传输（写寄存器）
    virtual bool write_register(uint8_t request, uint16_t value, uint16_t index, const uint8_t* data, uint16_t len) = 0;
    ...
    // 上载固件到芯片
    virtual bool upload_firmware(const uint8_t* fw_data, size_t fw_size, uint32_t base_address) = 0;
...
public:
    ...
    bool read_register(uint8_t request, uint16_t value, uint16_t index, uint8_t* data, uint16_t len) override {
        (void)request;
        (void)value;
        (void)index;
        (void)data;
    ...
    bool write_register(uint8_t request, uint16_t value, uint16_t index, const uint8_t* data, uint16_t len) override {
        (void)request;
        (void)value;
        (void)index;
        (void)data;
    ...
    bool upload_firmware(const uint8_t* fw_data, size_t fw_size, uint32_t base_address) override {
        (void)fw_data;
        (void)fw_size;
        (void)base_address;
        return true;
    ...
    bool is_firmware_loaded() const override {
        return firmware_loaded_;
    }

    const UsbWifiDeviceId& get_device_id() const override {
        return device_id_;
    }

    const UsbWifiCapability& get_capability() const override {
        return capability_;
    }

    // RTL 特有
    bool write_mac_register(uint8_t offset, uint32_t value) {
        return true;
        (void)offset;
        (void)value;
    }

    bool write_bb_register(uint8_t offset, uint32_t value) {
        return true;
        (void)offset;
        (void)value;
    }
...
public:
    ...
    bool enter_monitor_mode_8812() {
        // 1. 写入 RCR 配置为监控模式
        uint32_t rcr_value =
            RCR_AAP | RCR_APM | RCR_AM | RCR_AB | RCR_ACRC32 | RCR_ADF | RCR_ACF | RCR_AMF | RCR_APP_PHYSTS;
    ...
    bool enable_rf_8812() {
        // 打开射频路径 A 和 B
        return write_rf_register(0, 0x00, 0x00000) && write_rf_register(1, 0x00, 0x00000);
    }
...
public:
    ...
    Rtl8187lUsbDriver() {
        ...
        capability_ = {20,    // max_tx_power_dbm
                       14,    // max_channel_24ghz
                       0,     // max_channel_5ghz
                       0,     // max_channel_6ghz
                       1,     // num_tx_antennas
    ...
    bool enter_monitor_mode_8187() {
        ...
        return write_register(0x05, REG_8187L_RX_CONF, 0, reinterpret_cast<const uint8_t*>(&rx_conf), 4);
    }
};
...
public:
    ...
    static TaskControlBlock* create_driver_task(UsbWifiDriver* driver, uint8_t stack_size_pow2 = 14) {
        const size_t stack_size = static_cast<size_t>(1) << stack_size_pow2;
        uint32_t* stack = static_cast<uint32_t*>(KernelHeap::instance().allocate(stack_size));
        if (!stack)
            return nullptr;

        // 创建任务并传入驱动指针（通过 TaskNotify 通信）
        TaskControlBlock* tcb =
            Scheduler::instance().create_task(wifi_driver_task_entry_, stack, static_cast<uint32_t>(stack_size),
