...
public:
    ...
    PpgHeartRateFilter() noexcept : head_{0}, count_{0}, window_sum_{0}, iir_bpm_q8_{0} {
    ...
    [[nodiscard]] int32_t update(uint32_t raw_bpm) noexcept {
        ...
        if (iir_bpm_q8_ == 0) {
            ...
            iir_bpm_q8_ = (kAlphaNum * input_q8 + (kAlphaDen - kAlphaNum) * iir_bpm_q8_) / kAlphaDen;
        }
...
public:
    ...
    StepDetector() noexcept
        : total_steps_{0}, step_state_{State::stable}, time_since_step_ms_{kMinStepIntervalMs} // 允许第一步立即被计数
          ,
          current_peak_mag_{0}, peak_head_{0}, peak_count_{0}, peak_sum_{0}, dynamic_high_{kDefaultHighThresh},
...
private:
    ...
    [[nodiscard]] static int32_t approx_magnitude(int32_t ax, int32_t ay, int32_t az) noexcept {
        const int64_t sq =
            static_cast<int64_t>(ax) * ax + static_cast<int64_t>(ay) * ay + static_cast<int64_t>(az) * az;
        if (sq <= 0)
            return 0;
...
public:
    ...
    ActivityStateEngine() noexcept
        : state_{ActivityState::unknown}, filtered_bpm_{0}, still_seconds_{0}, cadence_head_{0}, cadence_sum_{0},
...
private:
    ...
    void resolve_state() noexcept {
        ...
        } else if (still_seconds_ >= kSleepStillSec && filtered_bpm_ > 0 && filtered_bpm_ < kSleepHRThreshold) {
...
public:
    ...
    ActivityState advance_activity(uint32_t delta_ms) noexcept {
        return activity_engine_.update(step_detector_.get_steps(), ppg_filter_.get_filtered_bpm(), delta_ms);
    }
