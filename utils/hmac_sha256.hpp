...
public:
    ...
    void finish(uint8_t digest[DIGEST_SIZE]) noexcept {
        ...
        for (int i = 7; i >= 0; i--) {
            buf_[56u + (7u - static_cast<unsigned>(i))] = static_cast<uint8_t>(bit_count_ >> (i * 8));
        }
        process_block(buf_);
...
private:
    ...
    void process_block(const uint8_t blk[BLOCK_SIZE]) noexcept {
        ...
        for (int i = 0; i < 16; i++) {
            W[i] = (static_cast<uint32_t>(blk[i * 4]) << 24) | (static_cast<uint32_t>(blk[i * 4 + 1]) << 16) |
                   (static_cast<uint32_t>(blk[i * 4 + 2]) << 8) | (static_cast<uint32_t>(blk[i * 4 + 3]));
        }
        for (int i = 16; i < 64; i++) {
...
public:
    ...
    static void compute(const uint8_t* key, size_t key_len, const uint8_t* msg, size_t msg_len,
    ...
    static bool constant_time_eq(const uint8_t* a, const uint8_t* b, size_t len) noexcept {
        uint8_t diff = 0;
