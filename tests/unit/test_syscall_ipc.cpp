...
TEST_F(SyscallIpcTest, SuccessfulIpcWithCapabilities) {
    ...
    sender->cspace[1].type = CapType::Endpoint;
    sender->cspace[1].rights = {0, 1, 0, 0}; // Write only
    sender->cspace[1].object = ep;
    
    // Receiver needs Read capability to Endpoint in slot 2
    receiver->cspace[2].type = CapType::Endpoint;
    receiver->cspace[2].rights = {1, 0, 0, 0}; // Read only
    receiver->cspace[2].object = ep;
...
TEST_F(SyscallIpcTest, CallFailsWithoutWriteRights) {
    sender->cspace[1].type = CapType::Endpoint;
    sender->cspace[1].rights = {1, 0, 0, 0}; // Read only, missing Write
    sender->cspace[1].object = ep;
    ...
    EXPECT_EQ(sender->ipc_state, IpcState::Ready); // State unchanged
}

TEST_F(SyscallIpcTest, ReceiveFailsWithoutReadRights) {
    receiver->cspace[2].type = CapType::Endpoint;
    receiver->cspace[2].rights = {0, 1, 0, 0}; // Write only, missing Read
    receiver->cspace[2].object = ep;
    ...
    EXPECT_EQ(receiver->ipc_state, IpcState::Ready); // State unchanged
}
...
TEST_F(SyscallIpcTest, CallFailsWithInvalidSlotOrType) {
    ...
    // Wrong type
    sender->cspace[1].type = CapType::Memory;
    sender->cspace[1].rights = {1, 1, 1, 0}; 
    sender->cspace[1].object = nullptr;
