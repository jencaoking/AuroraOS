...
inline uint32_t* init_task_stack(uint32_t* stack_top, void (*entry_point)(void)) {
    ...
    *stk = 0x01000000; // xPSR: 设置 Thumb 位 (T-bit)
    stk--;
    *stk = reinterpret_cast<uint32_t>(entry_point); // PC: 任务入口地址
    stk--;
    *stk = 0xFFFFFFFD; // LR (R14): 设为 EXC_RETURN，表明返回后使用 PSP，并且初始不包含扩展的 FPU 栈帧
    stk--;
    *stk = 0x12121212; // R12
    stk--;
    *stk = 0x03030303; // R3
...
// =====================================================================
// ARM PMSAv7 (Cortex-M3/M4/M7/M4F) 内存保护单元实现
...
inline void set_privilege(uint32_t privilege) {
    ...
    if (privilege) {
        control |= 1u; // Set nPRIV
    } else {
        control &= ~1u; // Clear nPRIV
    }
